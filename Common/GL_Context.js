...
"Assets/paper3.png", "Assets/paper4.png",
"Assets/hand.png", "Assets/cockroach.png",
"Assets/wood.png", "Assets/tatami.png", "Assets/carpet.png", "Assets/wall.png", "Assets/furniture.png", "Assets/desk.png"];
var shadowTextureColor, shadowTextureDepth, shadowBuffer, renderbuffer;

//var depthProjectionMatrix = perspective(60, 2 / 1, 1, 100);
var depthProjectionMatrix = ortho(-20, 20, -20, 20, 0, 40);
var depthViewMatrix = lookAt(vec3(20,30,20), vec3(20,0,20), vec3(0, 0, -1));
var depthViewProjection = mult(depthProjectionMatrix, depthViewMatrix);
...
function GL_Context( canvas_id )
	...
	// Special WebGL-only Initialization
		...
		//exttest = gl.getExtension("EXT_frag_depth");
		for( var i = 0; i < texture_filenames_to_load.length; i++ )
			initTexture( texture_filenames_to_load[i], true );
		
		var program = this.program = initShaders( gl, "vertex-shader", "fragment-shader" );											// Load shader from HTML elements
    ...
    // SHADOW TEXTURE and FRAMEBUFFER

		var depthTextureExt = gl.getExtension("WEBGL_depth_texture"); // 
		if (!depthTextureExt) { console.log("depth texture fail\n"); return; }
		
		shadowBuffer = gl.createFramebuffer();
		gl.bindFramebuffer(gl.FRAMEBUFFER, shadowBuffer);
		shadowBuffer.width = 1024;  // mmmm we're sampling with mipmaps so double this?
		shadowBuffer.height = 1024;
		
		shadowTextureColor = gl.createTexture();
		gl.bindTexture(gl.TEXTURE_2D, shadowTextureColor);
		gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
		gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
		//gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR_MIPMAP_NEAREST);
        //gl.texImage2D(gl.TEXTURE_2D, 0, gl.DEPTH_COMPONENT, shadowBuffer.width, shadowBuffer.height, 0, gl.DEPTH_COMPONENT, gl.UNSIGNED_SHORT, null
		gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, shadowBuffer.width, shadowBuffer.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
		textures["shadowTextureColor"] = {
		    id: shadowTextureColor,
		    loaded: true
		};

		shadowTextureDepth = gl.createTexture();
		gl.bindTexture(gl.TEXTURE_2D, shadowTextureDepth);
		gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
		gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
		gl.texImage2D(gl.TEXTURE_2D, 0, gl.DEPTH_COMPONENT, shadowBuffer.width, shadowBuffer.height, 0, gl.DEPTH_COMPONENT, gl.UNSIGNED_SHORT, null);
		textures["shadowTextureDepth"] = {
		    id: shadowTextureDepth,
		    loaded: true
        };
		gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, shadowTextureColor, 0);
		gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.TEXTURE_2D, shadowTextureDepth, 0);


		if(gl.checkFramebufferStatus(gl.FRAMEBUFFER) != gl.FRAMEBUFFER_COMPLETE){
			console.log("The shadow buffer is broken!", gl.checkFramebufferStatus(gl.FRAMEBUFFER), gl.FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT);
		}
		else console.log("Frame buffer is OK!");
		
		var shadowProgram = this.shadowProgram = initShaders( gl, "vertex-shader-shadow", "fragment-shader-shadow" );	
	...
	// GRAPHICS ADDRESSES
		...
		function Graphics_Addresses( program, shadowProgram )		// Find out the memory addresses internal to the graphics card of each of its variables, and store them here locally for the Javascript to use
		{	
			...
			this.texture_loc = gl.getUniformLocation(program, "texture");
			this.shadowTexture_loc = gl.getUniformLocation(program, "shadowTexture");
			this.shadow2_MVP_loc = gl.getUniformLocation(program, "shadowMVP");


			this.shadow_MVP_loc = gl.getUniformLocation(shadowProgram, "shadowMVP");
			this.shadow_USE_TEXTURE_loc = gl.getUniformLocation(shadowProgram, "USE_TEXTURE");
		}
		g_addrs = new Graphics_Addresses( program, shadowProgram );	
	}
	...
	GL_Context.prototype.startRender = function (time) {
