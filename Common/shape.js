	...
	shape.prototype.update_uniforms = function( model_transform, camera_transform, projection_transform )			// Send the current matrices to the shader
		{
                ...
                // Depth
				var depthMVP = mult(depthViewProjection, model_transform);
				gl.uniform1i(g_addrs.shadow_USE_TEXTURE_loc, 1);
				gl.uniformMatrix4fv(g_addrs.shadow_MVP_loc, false, flatten(depthMVP));  // this one goes to the depth shader
				gl.uniformMatrix4fv(g_addrs.shadow2_MVP_loc, false, flatten(depthMVP)); // this one goes to the regular shader
				
               
				gl.uniform4fv( g_addrs.lightPosition_loc, 	vec4( 10,30,12,0 ) );
				gl.uniform4fv(g_addrs.lightColor_loc, vec4(1, 1, 1, 1));
                
                // Bind texture locations
				gl.uniform1i( g_addrs.texture_loc, 0);  // texture unit 0
				gl.uniform1i(g_addrs.shadowTexture_loc, 1);  // texture unit 1

				// shininess = smoothness, pow(smoothness) => specular
				// diffuse = chalkiness
				// specular = shininess

	            // Generic Diffuse Shade
	            //gl.uniform1f ( g_addrs.ambient_loc, .4 );
	            //gl.uniform1f ( g_addrs.diffuse_loc,  1 );
	            //gl.uniform1f ( g_addrs.specular_loc, 0 );
	            //gl.uniform1f ( g_addrs.shininess_loc, .1);

	            // No Shine Shade
				//gl.uniform1f ( g_addrs.ambient_loc, .5 );
				//gl.uniform1f ( g_addrs.diffuse_loc,  1.0);
				//gl.uniform1f ( g_addrs.specular_loc, 0.0 );
				//gl.uniform1f ( g_addrs.shininess_loc, 0.0);
				
				// // Solid Color Shade
				 //gl.uniform1f ( g_addrs.ambient_loc, 1.0 );
				 //gl.uniform1f ( g_addrs.diffuse_loc,  0.0);
				 //gl.uniform1f ( g_addrs.specular_loc, 0.0 );
				 //gl.uniform1f ( g_addrs.shininess_loc, 0.0);	

                // Simple
				gl.uniform1f(g_addrs.ambient_loc, .8);
				gl.uniform1f(g_addrs.diffuse_loc,  .5);
	...
	shape.prototype.draw = function( model_transform, camera_transform, projection_transform, texture_filename )
		{
			...
			gl.bindTexture(gl.TEXTURE_2D, textures["shadowTextureDepth"].id);
...
// something is very wrong here... we lose the uv map				
function shape_from_file(filename)
	{
		...
		this.webGLStart = function(meshes)
			{
				...
				// {
					...
					// this.texture_coords.push( [meshes.mesh.,0])
		...
		//OBJ.downloadMeshes({
		//	'mesh' : filename,
		//	}, (function(self) { return bind( self, self.webGLStart ) }(this) )  );	// this is the most confusing thing
		OBJ.downloadMeshes({
			'mesh' : filename,
			}, (function(self) { return self.webGLStart.bind(self) }(this) )
		);	  
	}
...
cube.prototype.populate = function (recipient, points_transform) {
    ...
    for (var i = 0; i < 3; i++) {					// Build a cube by inserting six triangle strips into the lists.
        for (var j = 0; j < 2; j++) {
            ...
            var transform = mat4();
            transform = mult(translate(-.5, 0, 0), transform);                        // Left
            transform = mult(rotate(180 * j, vec3(0, 0, 1)), transform);              // Right if j
...
function newspaper0( segments, near, far, wnear, wfar, hnear, hfar )	
{	
	shape.call(this);	
		
    this.populate( this, segments, near, far, wnear, wfar, hnear, hfar );
...
inherit(newspaper0, triangle_strip);

newspaper0.prototype.test = function () {
    console.log("tst ok!\n");
}
newspaper0.prototype.populate = function (recipient, segments, n, f, wn, wf, hn, hf) {
    // essentially a symmetrical fructrum
    var near = n;
    var far = f;
    var widthFar = wf;
    var widthNear = wn;
    ...
    var middleFar = widthFar / 2;
    var middleNear = widthNear / 2;
    for(var i=0; i<=segments; i++){
        // i * widthNear / segments;
        // i * widthFar / segments;
        vertices.push(vec3(i * widthNear / segments - middleNear, i % 2 * heightNear, -near));
        vertices.push(vec3(i * widthFar / segments - middleFar, i % 2 * heightFar, -far));

        // texturec[i * 2] =     vec2(0, i / segments);
        // texturec[i * 2 + 1] = vec2(1, i / segments);

        texturec.push(vec2(0, i / segments));
        texturec.push(vec2(1, i / segments));

        strip_indices.push(i*2);     // Near
        strip_indices.push(i*2 + 1); // Far
        // i%2 = down, up, down, up, ...
    }

    for (var i = 0; i < segments; i++) {
        recipient.vertices.push(      vertices[2 * i + 0]);
        recipient.vertices.push(      vertices[2 * i + 1]);
        recipient.vertices.push(      vertices[2 * i + 2]);
        recipient.texture_coords.push(texturec[2 * i + 0]);
        recipient.texture_coords.push(texturec[2 * i + 1]);
        recipient.texture_coords.push(texturec[2 * i + 2]);

        recipient.vertices.push(      vertices[2 * i + 1]);
        recipient.vertices.push(      vertices[2 * i + 3]);
        recipient.vertices.push(      vertices[2 * i + 2]);
        recipient.texture_coords.push(texturec[2 * i + 1]);
        recipient.texture_coords.push(texturec[2 * i + 3]);
        recipient.texture_coords.push(texturec[2 * i + 2]);

        recipient.indices.push(6 * i + 0);
        recipient.indices.push(6 * i + 1);
        recipient.indices.push(6 * i + 2);
        recipient.indices.push(6 * i + 3);
        recipient.indices.push(6 * i + 4);
        recipient.indices.push(6 * i + 5);
    }
    recipient.flat_normals_from_triples(0);

    //// this method uses smooth shading :/, might have to do it by hand
    //this.init_from_strip_lists(recipient, vertices, strip_indices);
    //recipient.flat_normals_from_triples(0);
